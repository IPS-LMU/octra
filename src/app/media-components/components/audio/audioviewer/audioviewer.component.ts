  ...
  public selectSegment(segIndex: number, successcallback: (posY1: number, posY2: number) => void = () => {
    ...
    if (segIndex > -1) {
      ...
      if (startTime.browserSample.value >= this.audiochunk.time.start.browserSample.value
        ...
        console.log(`audiochunk time: (${this.audiochunk.time.start.browserSample.value} - ${this.audiochunk.time.end.browserSample.value})\nsegment time: (${startTime.browserSample.value} - ${segment.time.browserSample.value})`);
        errorcallback();
      }
      return false;
    } else {
    ...
    if (this.av.viewRect.size.height >= this.av.viewRect.size.height) {
      if ($event.type === 'mousemove') {
        if (this.scrollbar.dragging) {
          this.scrollTo((($event as any).layerY / this.av.viewRect.size.height) * this.av.realRect.size.height, true);
        }
        ...
        if (!this.scrollbar.dragging) {
          this.scrollTo((($event as any).layerY / this.av.viewRect.size.height) * this.av.realRect.size.height, false);
        }
        this.scrollbar.dragging = false;
      }
    }
