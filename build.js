...
async function main() {
  ...
  const command = ['./node_modules/nx/dist/bin/nx.js', 'build', 'octra', '--prod', dev, `--base-href=${baseHref}`, `--deploy-url=assets/`, '--skip-nx-cache'];
  ...
  node.on('exit', async function (code) {
    ...
    // change build dir
    execSync(`mv ./${buildDir}browser/* ./${buildDir}`);
    execSync(`rm -rf ./${buildDir}browser`);
