...
export class AudioviewerService extends AudioComponentService {
	...
	public overboundary: boolean = false;
	...
	public shift_pressed: boolean = false;
	...
	public updatePlayDuration() {
		if (this.selection && this.selection.start.samples < this.selection.end.samples) {
			...
			let start_pos = this.current.samples;
	...
	changeBuffer(start: AudioTime, end: AudioTime, innerWidth: number) {
		...
		if (this._channel.length == 0)
			throw "Channel Data Length is 0";
		else {
			this.DurTime = new AudioTime(this.Chunk.time.length, this.audio.samplerate);
			this.afterChannelInititialized(innerWidth);
	...
	private calculateZoom(height: number, width: number, minmaxarray: number[]) {
		if (this.Settings.justify_signal_height) {
			...
			if (max_zoom_y > 0) {
				...
				console.log("calc zoom: " + this._zoomY);
	...
	public setMouseMovePosition(type: string, x: number, y: number, curr_line: Line, innerWidth) {
		...
		if (this.mouse_down && this.dragableBoundaryNumber < 0) {
			...
			let segment = this.transcrService.segments.get(this.dragableBoundaryNumber);
	...
	public setMouseClickPosition(x: number, y: number, curr_line: Line, $event: Event, innerWidth: number, callback = () => {
		...
		let absX = this.getAbsXByLine(curr_line, x - curr_line.Pos.x, innerWidth);
		let absXInTime = this.audioTCalculator.absXChunktoSamples(absX, this.Chunk);
	...
	public addSegment(): { type: string, seg_num: number, msg: { type: string, text: string } } {
		...
		if (line && this.Settings.boundaries.enabled && !this.Settings.boundaries.readonly) {
			let absXTime = (!this.audio.audioplaying) ? this.mousecursor.timePos.samples : this.PlayCursor.time_pos.samples;
			...
			if (this.transcrService.segments.length > 0) {
				for (i = 0; i < this.transcrService.segments.length; i++) {
					if ((this.transcrService.segments.get(i).time.samples >= absXTime - b_width_time
						...
						let seg_after = (i < this.transcrService.segments.length - 1) ? this.transcrService.segments.get(i + 1) : null;
			...
			let selection: number = Math.abs(this.selection.end.samples - this.selection.start.samples);

			if (selection > 0) {
				//some part selected
				let segm1 = this.transcrService.segments.BetweenWhichSegment(this.selection.start.samples);
				let segm2 = this.transcrService.segments.BetweenWhichSegment(this.selection.end.samples);
				...
				let segment = this.transcrService.segments.BetweenWhichSegment(Math.round(absXTime));
	...
	public getRelativeSelectionByLine(line: Line, start_samples: number, end_samples: number, innerWidth: number): { start: number, end: number } {
		...
		if (selection) {
			if (!line)
				throw "line null";
			let absX = line.number * innerWidth;
			let absEnd = absX + line.Size.width;
			let SelAbsStart = this.audioTCalculator.samplestoAbsX(start_samples - this.Chunk.time.start.samples);
			let SelAbsEnd = this.audioTCalculator.samplestoAbsX(end_samples - this.Chunk.time.start.samples);
	...
	public updateLines(innerWidth: number) {
		if (this.Settings.multi_line) {
			let lines = Math.ceil(this.audio_px_w / innerWidth);
			...
			for (let i = 0; i < lines; i++) {
				let w = (rest_width > innerWidth) ? innerWidth : rest_width;
	...
	getLineByAbsX(absX, innerWidth): Line {
		let line_num = Math.floor(absX / innerWidth);
	...
	addLine(line_num: number, w: number, h: number) {
		...
		var line_obj = new Line(line_num, size, position);
	...
	getBoundaryNumber(absX: number): number {
		...
		for (let i = 0; i < this.transcrService.segments.length - 1; i++) {
			...
			let segAbsX = this.audioTCalculator.samplestoAbsX(segment.time.samples - this.Chunk.time.start.samples);
			let next_unblocked = true;
			if (i < this.transcrService.segments.length - 1) {
				let next_segment = this.transcrService.segments.get(i + 1);
	...
	public moveCursor(direction: string, samples: number) {
			...
			if ((direction === "left" || direction === "right") &&
				...
				if (direction === "left") {
					if (this.mousecursor.timePos.samples >= this.Chunk.time.start.samples + samples) {
						...
						let rel_samples = this.mousecursor.timePos.samples - this.Chunk.time.start.samples;
						let absx = this.audioTCalculator.samplestoAbsX(rel_samples, this.Chunk.time.duration);
		...
		else throw "can not move cursor by given samples. Number of samples less than 0.";
	}
	...
	private validateConfig() {
		let validator: AudioviewerConfigValidator = new AudioviewerConfigValidator();
