...
export class Segments {
	...
	constructor(private sample_rate, segments: any[], last_sample: number) {
		...
		for (let i = 0; i < segments.length; i++) {
			let new_segment = Segment.fromAny(segments[ i ]);
	...
	public add(time_samples: number): boolean {
		let newSegment: Segment = new Segment(new AudioTime(time_samples, this.sample_rate));
	...
	public removeByIndex(index: number) {
		if (index > -1 && index < this.segments.length) {
			if (index < this.segments.length - 1) {
				let next_segment = this.segments[ index + 1 ];
				let transcription: string = this.segments[ index ].transcript;
	...
	public sort() {
		this.segments.sort(function (a, b) {
			if (a.time.samples < b.time.samples) return -1;
			if (a.time.samples == b.time.samples) return 0;
			if (a.time.samples > b.time.samples) return 1;
		});
	...
	public getStartTime(id: number): AudioTime {
		let segment = this.get(id);
		let samples = 0;

		if (segment) {
			for (let i = 0; i < this.segments.length; i++) {
				if (id === i) {
					let res = segment.time.clone();
					res.samples = samples;
					return res;
				}
