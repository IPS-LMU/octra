...
export class JSONSetValidator {
  ...
  validate(set: unknown[], setSchema: JSONSetDefinition): JSONSetValidationError[] {
    throw new Error("Missing implementation");
  }

  protected __validate(target: unknown, constraints: JSONSetConstraints, path: string): JSONSetValidationError[] {
    return [
      {
        path,
...
export class FileJSONSetValidator extends JSONSetValidator {
  ...
  override validate(set: File[], setSchema: JSONFileSetDefinition): JSONFileSetValidationError[] {
    const results: {
    ...
    for (const statement of setSchema.statements) {
      results.push(
        this.getValidFileNamesOfSelection(set, statement, [setSchema.name, statement.name].filter(a => a).join("."))
      );
    }
  ...
  override __validate(target: File, constraints: JSONSETFileConstraints, path: string): JSONFileSetValidationError[] {
    if (!target) {
      return [
        {
          filename: "",
    ...
    if (target) {
      ...
      if (constraints.extension && constraints.extension.length > 0) {
        if (!constraints.extension.find(a => target.name.indexOf(a) === target.name.length - a.length)) {
          errors.push({
            filename: target.name,
            path,
      ...
      if (constraints.mimeType && constraints.mimeType.length > 0) {
        if (!constraints.mimeType.includes(target.type)) {
          errors.push({
            ...
            path,
            constraint: "mimeType",
            message: `MIME-Type ${target.type} does not match [${constraints.mimeType.join(",")}].`
          });
        }
      }
  ...
  private convertFileString(fileString: string) {
    const matches = /\s*([0-9]+(?:\.?[0-9]+)?)\s?((?:B)|(?:KB)|(?:MB)|(?:TB))$/g.exec(fileString);
  ...
  private getValidFileNamesOfSelection(pool: File[], statement: JSONFileSetStatement, path: string) {
    const result: {
      statement: JSONSetStatement;
      validationResults: {
        filename: string;
    ...
    for (const file of pool) {
      if (statement.constraints) {
        ...
        for (const ofElement of statement.constraints) {
          const errors = this.__validate(file, (ofElement as JSONSETFileConstraints), path);
  ...
  private analyzeValidationResults(validations: {
    statement: JSONSetStatement;
    validationResults: {
      filename: string;
      isTaken?: boolean;
    ...
    const takesTable: number[] = validations.map(a => (a.statement.take ?? a.statement.takeMin ?? a.statement.takeMax ?? 0));
    ...
    const noRemainingTakesOrSets = () => {
      return takesTable.filter(a => a > 0).length === 0 ||
        quantity.filter(a => a.score > 0).length === 0;
    };

    while (!noRemainingTakesOrSets()) {
      let setIndexWithExactOneFile = validations.findIndex(a => a.validationResults.filter(a => a.errors.length === 0).length === 1);


      // clear sets with only one file
      while (setIndexWithExactOneFile > -1) {
        const validFiles = validations[setIndexWithExactOneFile].validationResults.filter(a => a.errors.length === 0);
        const filename = validFiles[0].filename;
        takeFileFromSet(filename, setIndexWithExactOneFile);
        setIndexWithExactOneFile = validations.findIndex(a => a.validationResults.filter(a => a.errors.length === 0 && !a.isTaken).length === 1);
      }
    ...
    for (let i = 0; i < takesTable.length; i++) {
      ...
      if (numberOfTakes > 0) {
        if (statement.take) {
          ...
          // success
          validations[i].validationResults = validations[i].validationResults.map(a => ({
            ...a,
            isTaken: true
          }));
        }
      } else if (validations[i].validationResults.filter(a => a.errors.length === 0 && !a.isTaken).length > 0) {
    ...
    const remainingErrors: JSONFileSetValidationError[] = validations.map(a => a.validationResults.filter(a => a.errors.length > 0 && !a.isTaken).map(b => b.errors)).flat(2);
  ...
  private calculateQuantityTable(validations: {
    statement: JSONSetStatement;
    validationResults: {
      filename: string;
      isTaken?: boolean;
    ...
    for (const validation of validations) {
      for (const validResult of validation.validationResults) {
        if (validResult.errors.length === 0) {
          const index = result.findIndex(a => a.filename === validResult.filename);
          if (index > -1) {
