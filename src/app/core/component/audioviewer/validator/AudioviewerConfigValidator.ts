import {ConfigValidator, ValidationResult} from '../../../obj/ConfigValidator';
...
export class AudioviewerConfigValidator extends ConfigValidator {
  ...
  public validate(key: string, value: any): ValidationResult {
    const prefix = 'AudioviewerConfig Validation - ';
    ...
    switch (key) {
      ...
      case('timeline'):
        if (typeof value === 'object' && typeof value.enabled === 'boolean' &&
          ...
          typeof value.fontSize === 'number' && typeof value.fontWeight === 'string' &&
          typeof value.font === 'string' && typeof value.foreColor === 'string'
        ) {
          break;
        }
        ...
        return {
          success: false,
          error: prefix + 'value of key \'' + key + '\' must be an object of width attributes ' +
          '{enabled:boolean, step_width_ratio: boolean, height:string, fontSize:number, fontWeight:string, font:string, foreColor:string}'
        };
