...
export class AudioviewerComponent implements OnInit, OnDestroy, AfterViewInit {
	...
	private _deactivate_shortcuts: boolean = false;
	...
	drawSignal = function (line_num) {
		//get canvas
		let line_obj = this.av.LinesArray[ line_num ];
		let timeline_height = (this.Settings.timeline.enabled) ? this.Settings.timeline.height : 0;

		if (line_obj) {
			//line_obj found
			let midline = Math.floor((this.Settings.height - timeline_height) / 2);
	...
	drawGrid(hLines: number, vLines: number, line: Line) {
		let timeline_height = (this.Settings.timeline.enabled) ? this.Settings.timeline.height : 0;
		let hZoom = Math.round(this.innerWidth / vLines);
		let vZoom = Math.round((this.Settings.height - timeline_height) / hLines);
	...
	clearDisplay(line_num) {
		...
		if (line_obj) {
			...
			this.g_context.fillRect(line_obj.Pos.x, line_obj.Pos.y + (this.Settings.height - 2 - timeline_height), line_obj.Size.width, timeline_height);
	...
	drawSegments() {
		...
		if (this.Settings.boundaries.enabled && this.transcr.segments) {
			let segments = this.transcr.segments.getSegmentsOfRange(this.av.Chunk.time.start.samples, this.av.Chunk.time.end.samples);
			this.o_context.globalAlpha = 1.0;

			for (let i = 0; i < segments.length; i++) {
				let segment = segments[ i ];
				let start = AudioTime.sub(segments[ i ].time, this.av.Chunk.time.start);
				let absX = this.av.audioTCalculator.samplestoAbsX(start.samples);
				let begin = new Segment(new AudioTime(0, this.audio.samplerate));

				if (i > 0) begin = segments[ i - 1 ];
				let beginX = this.av.audioTCalculator.samplestoAbsX(begin.time.samples);
				let line_num1 = (this.innerWidth < this.AudioPxWidth) ? Math.floor(beginX / this.innerWidth) : 0;
				let line_num2 = (this.innerWidth < this.AudioPxWidth) ? Math.floor(absX / this.innerWidth) : 0;

				for (let j = 0; j <= line_num2; j++) {
					let line = this.av.LinesArray[ line_num1 + j ];
					...
					if (line) {
						...
						let select = this.av.getRelativeSelectionByLine(line, begin.time.samples, segments[ i ].time.samples, this.innerWidth);
						let w = 0;
						let x = select.start;
				...
				//draw boundaries
				let line = this.av.LinesArray[ line_num2 ];
		...
		for (let j = 0; j < this.av.LinesArray.length; j++) {
			let line = this.av.LinesArray[ j ];

			let select = this.av.getRelativeSelectionByLine(line, this.Selection.start.samples, this.Selection.end.samples, this.innerWidth);
			if (select && line) {
				let left = select.start;
				let right = select.end;
						...
						switch (shortc) {
							...
							case("play_selection"):
								if (this.av.focused) {
									...
									let boundary_select = this.av.getSegmentSelection(this.av.Mousecursor.timePos.samples);
									if (boundary_select) {
	...
	drawPlayCursor = () => {
		...
		let calculator = new AudioTimeCalculator(this.audio.samplerate, this.audio.duration, this.AudioPxWidth);

		let duration = (this.av.Selection.length > 0) ? new AudioTime(this.av.Selection.length, this.audio.samplerate) : this.av.playduration;
		let currentAbsX = this.av.audioTCalculator.samplestoAbsX((this.av.current.samples - this.av.Chunk.time.start.samples));
		let progress = 0;
		let absX = 0;
	...
	drawPlayCursorOnly = (curr_line: Line) => {
		if (curr_line) {
			let player_width = this.Settings.playcursor.width;

			let relX = this.av.PlayCursor.absX - (curr_line.number * this.innerWidth);
			let relY = curr_line.Pos.y;
	...
	drawTimeLine = function () {
		if (this.Settings.timeline.enabled && this.av.LinesArray.length > 0
			...
			let sec_px = (this.Settings.multi_line)
			...
			let parts = this.av.AudioPxWidth / max_width;
			let start_time: AudioTime = this.av.Chunk.time.start;
			...
			if (Number.isFinite(parts)) {
				...
				for (let k = 0; k < parts - 1 && sum_width < this.av.AudioPxWidth; k++) {
					let line_num = Math.floor(sum_width / this.innerWidth);
					let line_obj = this.av.LinesArray[ line_num ];
					let seconds = start_time.seconds + k * mwidth_seconds;
					let time2 = AudioTime.fromSeconds(seconds, this.audio.samplerate);
					...
					this.g_context.beginPath();
					this.g_context.fillText(seconds, line_obj.Pos.x + relX + 6, line_obj.Pos.y + line_obj.Size.height - ((this.Settings.timeline.height - this.Settings.timeline.fontSize)));
			...
			else Logger.err("Audioviewer, number of parts of timeline is infinite!");
	...
	public selectSegment(seg_index: number, successcallback: (posY1: number, posY2: number) => void = () => {
		...
		if (seg_index > -1) {
			let segment = this.transcr.segments.get(seg_index);
			let start_time = this.transcr.segments.getStartTime(seg_index);
			//make shure, that segments boundaries are visible
			if (start_time.samples >= this.av.Chunk.time.start.samples && segment.time.samples <= this.av.Chunk.time.end.samples) {
				let absX = this.av.audioTCalculator.samplestoAbsX(this.transcr.segments.get(seg_index).time.samples);
				let begin = new Segment(new AudioTime(0, this.audio.samplerate));
				if (seg_index > 0)
				...
				let boundary_select = this.av.getSegmentSelection(segment.time.samples - 1);
				if (boundary_select) {
					this.av.Selection = boundary_select;
