    ...
    <?php if (sizeof($screenshots) > 0): ?>
      ...
      <div class="screenshots">
        <?php foreach ($screenshots as $screenshot): ?>
          <img class="screenshot" src="data:image/png;base64,<?php echo $screenshot["data"]; ?>" alt="scrrenshot"/>
          <br/>
        <?php endforeach; ?>
      </div>
    <?php endif; ?>
      ...
      <table>
        ...
        foreach ($data["report"]["entries"] as $entry):
          ...
          <tr>
            ...
            <td
              class="date-col"><?php echo (!empty($entry["timestamp"])) ? $entry["timestamp"] : "No date recorded"; ?></td>
            <td class="type-col">
...
if (!empty($data) && !is_null($headers["Authorization"]) && $headers["Authorization"] === $auth_token) {
  ...
  if (!is_null($data["additional_information"]) && !is_null($data["description"])) {
    ...
    if (!empty($data["additional_information"]["name"])) {
      $from_name = $data["additional_information"]["name"];
    } else {
      $from_name = "";
    }
    ...
    if ($from_name != "") {
      $header = "From: \"$from_name\" <" . $from_email . ">" . $eol;
    } else {
