...
export class TranscrEditorComponent implements OnInit, OnDestroy, OnChanges {
  ...
  initPopover() {
    // set popover
    jQuery('.btn-icon-text[data-boundary]').off('click');
    setTimeout(() => {
      jQuery('.btn-icon-text[data-boundary]').on('click', function (event) {
        const jqueryobj = jQuery(this);
        // alert('segment ' + jqueryobj.attr('data-boundary') + ' clicked!');
        jqueryobj.css({
          'background-color': '#5add80'
        });
      });
      jQuery('.btn-icon-text[data-boundary]').on('mouseover', function (event) {
        console.log(event);

        const test = jQuery('<div></div>');
        test.css({
        ...
        jQuery(event.target).append(test);
  ...
  createCustomButtonsArray(): any[] {
    ...
    const boundary_btn = (context) => {
      ...
      const btn_js = {
        ...
        tooltip: 'test',
        click: () => {
          this.insertBoundary('assets/img/default/favicon.ico', 0);
          this.initPopover();
        }
      };
      const button = jQuery.summernote.ui.button(btn_js);
