...
export class AsrService {
  ...
  public init() {
    this._queue = new ASRQueue(
      this.asrSettings,
      this.audioService.audiomanagers[0],
      this.httpClient
    );
    if (
      ...
      this._selectedLanguage = this.getLanguageByCode(
        this.appStorage.asrSelectedLanguage,
        this.appStorage.asrSelectedService
      );
    }
  }
  ...
  public addToQueue(
    ...
    const asrInfo = this.asrSettings.services.find(
      (a) => this.selectedLanguage.asr === a.provider
    );
...
class ASRQueue {
  ...
  public add(queueItem: ASRQueueItem) {
    ...
    if (!found) {
      ...
      console.error(
        new Error('QueueItem with id ' + queueItem.id + ' already added!')
      );
    }
  }
  ...
  public remove(id: number) {
    ...
    if (index > -1) {
      ...
      console.error(
        `queueItem with id ${id} does not exist and can't be removed.`
      );
    }
  }
  ...
  public getItemByTime(
    sampleStart: number,
    sampleLength: number
  ): ASRQueueItem | undefined {
      ...
      if (this._statistics.running < this.MAX_PARALLEL_ITEMS) {
        ...
        if (nextItem !== undefined) {
          if (nextItem.startProcessing()) {
            ...
            nextItem.statusChange.subscribe(
              (status) => {
                if (
                  status.new !== ASRProcessStatus.STARTED &&
                  status.new !== ASRProcessStatus.NOAUTH &&
                  status.old !== ASRProcessStatus.NOAUTH &&
                  status.new !== ASRProcessStatus.RUNNING
                ) {
  ...
  private updateStatistics(status: {
    ...
    if (
      status.old === ASRProcessStatus.STARTED &&
      status.new === ASRProcessStatus.RUNNING
    ) {
      // increment to fix decrementing running
      this._statistics.running += 1;
    } else if (
      status.old === ASRProcessStatus.RUNNING &&
      status.new !== ASRProcessStatus.RUNNING
    ) {
      this._statistics.running = Math.max(0, this._statistics.running - 1);
    }
...
export class ASRQueueItem {
  ...
  public startProcessing(): boolean {
    if (this.status !== ASRProcessStatus.STARTED) {
      if (this._type === ASRQueueItemType.ASR) {
        this.transcribeSignalWithASR('txt')
          .then(() => {
            this._progress = 1;
        ...
        this.transcribeSignalWithASR('txt')
          .then((asrResult) => {
            ...
            this.callMAUS(
              ...
              asrResult.audioURL,
              asrResult.transcriptURL
            )
              .then(this.readMAUSResult)
              .catch(this.onMAUSRequestError);
          })
          .catch((error) => {
            console.error(error);
          });
      } else if (this._type === ASRQueueItemType.MAUS) {
        this.processWithMAUSONLY()
          .then(this.readMAUSResult)
          .catch((error) => {
            console.error(error);
          });
  ...
  private readMAUSResult = (mausResult: { file: File; url: string }) => {
    this.readTextFromFile(mausResult.file)
      .then((result) => {
        this._result = result;
        this._progress = 1;
        ...
        .then((file) => {
          if (this._status !== ASRProcessStatus.STOPPED) {
            ...
            const serviceRequirementsError = this.fitsServiceRequirements(
              new File([file.uint8Array], file.fileName, { type: 'audio/wav' })
            );
            if (serviceRequirementsError === '') {
              // 2) upload signal
              this.uploadFile(
                new File([file.uint8Array], file.fileName, {
                  type: 'audio/wav',
                }),
                this.selectedLanguage
              )
                .then((audioURL: string) => {
                  if (this._status !== ASRProcessStatus.STOPPED) {
                    ...
                    this.callASR(this.selectedLanguage, audioURL, outFormat)
                      .then((asrResult) => {
                        if (this._status !== ASRProcessStatus.STOPPED) {
                          this.readTextFromFile(asrResult.file)
                            .then((result) => {
                              this._result = result;
          ...
          if (this._status !== ASRProcessStatus.STOPPED) {
            ...
            if (serviceRequirementsError === '') {
              ...
              const transcriptFile = new File(
                [this._transcriptInput],
                `OCTRA_ASRqueueItem_${this._id}.txt`,
                { type: 'text/plain' }
              );
              promises.push(
                this.uploadFile(transcriptFile, this.selectedLanguage)
              );
              promises.push(
                this.uploadFile(
                  new File([file.uint8Array], file.fileName, {
  ...
  private fitsServiceRequirements(file: File): string {
    if (this._selectedASRInfo !== undefined) {
      if (this._selectedASRInfo.maxSignalDuration !== undefined) {
        if (
          this.time.sampleLength / this.sampleRate >
          this._selectedASRInfo.maxSignalDuration
        ) {
  ...
  private callASR(
    ...
    return new Promise<{
      ...
      const asrUrl = this.parent.asrSettings.calls[0]
  ...
  private callMAUS(
    ...
    return new Promise<{
      ...
      const mausURL = this.parent.asrSettings.calls[1]
        .replace('{{host}}', languageObject.host)
        .replace('{{audioURL}}', audioURL)
  ...
  private extractResultData = (
    ...
    return new Promise<{ file: File; url: string }>((resolve, reject) => {
      ...
      if (json.success === 'true') {
        ...
        file
          .updateContentFromURL(this.parent.httpClient)
          .then(() => {
