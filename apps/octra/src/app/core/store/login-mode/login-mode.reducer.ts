...
import { ASRContext, OctraAnnotation, OctraAnnotationSegment } from '@octra/annotation';
...
export class LoginModeReducers {
  ...
  public create(): ActionReducer<AnnotationState, Action> {
    return createUndoRedoReducer(
      ...
      on(LoginModeActions.clearWholeSession.success, (state: AnnotationState, { mode }) => {
        if (this.mode === mode) {
          return {
            ...initialState,
          };
      ...
      on(LoginModeActions.clearOnlineSession.do, (state: AnnotationState, { mode }) => {
        if (this.mode === mode) {
          return {
            ...initialState,
      ...
      on(LoginModeActions.redirectToProjects.do, (state: AnnotationState, { mode }) => {
        if (this.mode === mode) {
          return {
            ...state,
            savingNeeded: false,
      ...
      on(LoginModeActions.quit.do, (state: AnnotationState, { clearSession, freeTask, mode }) => {
        if (this.mode === mode) {
      ...
      on(
        ...
        (state: AnnotationState, { clearSession, mode, keepPreviousInformation }) => {
          if (this.mode === mode) {
            return clearSession
              ? {
                  ...initialState,
                  currentEditor: state.currentEditor,
                  currentSession: keepPreviousInformation ? state.currentSession : initialState.currentSession,
                  previousCurrentLevel: keepPreviousInformation ? state.previousCurrentLevel : initialState.previousCurrentLevel,
                  previousSession: keepPreviousInformation ? state.previousSession : initialState.previousSession,
                }
              : {
                  ...state,
                  savingNeeded: false,
      ...
      on(LoginModeActions.setFeedback, (state: AnnotationState, { feedback, mode }) => {
        if (mode === this.mode) {
          return {
            ...state,
            currentSession: {
      ...
      on(LoginModeActions.changeComment.do, (state: AnnotationState, { comment, mode }) => {
        if (this.mode === mode) {
          return {
            ...state,
            currentSession: {
      ...
      on(IDBActions.loadOptions.success, (state: AnnotationState, { onlineOptions, demoOptions, localOptions }) => {
        let result = state;
      ...
      on(LoginModeActions.loadProjectAndTaskInformation.success, (state: AnnotationState, { currentProject, task, mode }) => {
        if (this.mode === mode) {
          return {
            ...
            logging: {
              ...state.logging,
              enabled: (task?.tool_configuration?.value as ProjectSettings)?.logging?.forced === true ? true : state.logging.enabled,
            },
          };
        }
        return state;
      }),
      on(LoginModeActions.loadProjectAndTaskInformation.do, (state: AnnotationState, { mode }) => {
        if (this.mode === mode) {
          return {
            ...state,
            currentSession: {
      ...
      on(LoginModeActions.startAnnotation.do, (state: AnnotationState, { mode }) => {
        if (this.mode === mode) {
          return {
            ...state,
      ...
      on(AuthenticationActions.loginLocal.prepare, (state: AnnotationState, { mode, sessionFile, removeData, files, annotation }) => {
        if (this.mode === mode) {
          ...
          if (this.mode === mode) {
            return {
              ...
              logging: {
                ...
                startTime: Date.now(),
                startReference: state.logging.logs && state.logging.logs.length > 0 ? state.logging.logs[state.logging.logs.length - 1] : undefined,
              },
              currentSession: {
                ...state.currentSession,
                loadFromServer: true,
      ...
      on(LoginModeActions.changeImportOptions.do, IDBActions.loadImportOptions.success, (state: AnnotationState, { mode, importOptions }) => {
        if (mode === this.mode) {
          return {
            ...state,
            importOptions,
      ...
      on(LoginModeActions.setImportConverter.do, (state: AnnotationState, { mode, importConverter }) => {
        if (mode === this.mode) {
          return {
            ...state,
            importConverter,
      ...
      on(ApplicationActions.setAppLanguage, (state: AnnotationState, { language }) => {
        if (state.guidelines?.list && state.guidelines?.list.length > 0) {
          let guideline = state.guidelines.list.find((a) => a.filename === `guidelines_${language}.json`);
          // fallback to english
          guideline = guideline ?? state.guidelines.list.find((a) => a.filename === `guidelines_en.json`);
      ...
      on(AnnotationActions.regReplaceModal.open, (state: AnnotationState) => ({
        ...state,
        modalVisibilities: {
          ...state.modalVisibilities,
          regReplace: true,
      ...
      on(AnnotationActions.regReplaceModal.close, (state: AnnotationState) => ({
        ...state,
        modalVisibilities: {
          ...state.modalVisibilities,
          regReplace: false,
      ...
      on(AnnotationActions.combineTranscriptsModal.open, (state: AnnotationState) => ({
        ...state,
        modalVisibilities: {
          ...state.modalVisibilities,
      ...
      on(AnnotationActions.combineTranscriptsModal.close, (state: AnnotationState) => ({
        ...state,
        modalVisibilities: {
          ...state.modalVisibilities,
      ...
      on(AnnotationActions.cuttingModal.open, (state: AnnotationState) => ({
        ...state,
        modalVisibilities: {
          ...state.modalVisibilities,
          cutting: true,
      ...
      on(AnnotationActions.cuttingModal.close, (state: AnnotationState) => ({
        ...state,
        modalVisibilities: {
          ...state.modalVisibilities,
          cutting: false,
